...
export default class App {
	...
	private setupErrorHandlers() {
		this.app.use((_: express.Request, response: express.Response) => { response.status(404).send('Not found') })
		this.app.use((error: Error, _: express.Request, response: express.Response, __: express.NextFunction) => {
			log.error('[App] An error occurred:', error)
			response.status(500).json({ error: error.message ?? 'Internal server error', stack: error.stack })
		})
